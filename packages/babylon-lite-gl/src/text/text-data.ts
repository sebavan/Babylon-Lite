...
import { disposeBuffer, type GLVertexBuffer } from "../mesh.js";
...
export type TextDataDrawGroup = {
    ...
    freeSlots: number[];
    /** Lazy GPU draw state for this group's atlas (recreated on atlas-grow or first bind).
     *  WebGL2 analog of the WebGPU per-group bind group — a VAO capturing the instance
     *  attribute pointers (plus the curve/band texture units bound at draw time). Phase 2:
     *  populated by the WebGL2 draw path; stays `null` in the CPU-only Phase 1. */
    bindGroup: WebGLVertexArrayObject | null;
 ...
 *  Phase 2: the instance-buffer creation + upload (the WebGPU `device.createBuffer` /
 *  `queue.writeBuffer` path) is not ported yet; in Phase 1 `TextData._gpu` stays `null` and
 *  only the CPU `_instances` Float32Array is built/maintained. */
export type TextDataGpu = {
    engine: GLEngineContext;
    instanceBuf: GLVertexBuffer;
...
export function disposeTextData(data: TextData): void {
    if (data._gpu) {
        disposeBuffer(data._gpu.engine, data._gpu.instanceBuf);
        data._gpu = null;
    }
    for (const g of data._groups) {
        g.bindGroup = null;
