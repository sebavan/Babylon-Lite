 ...
 *    - GPU creation/upload (`SharedAtlasGpu`) is the WebGL2 float-texture upload path.
 *      Phase 2: that upload is not ported yet (its WebGPU counterpart lived in
 *      `_gpu/text-textures.ts`); the shared `SharedAtlasGpu` type is re-typed to lite-gl
 *      handles and exported from here. GPU teardown is performed inline in
 *      `disposeGlyphStorage` via `disposeTexture` (no circular import edge).
 *
...
import { disposeTexture, type GLTexture } from "../texture.js";
...
/** @internal GPU-side companion to a `SharedAtlas`; populated lazily by the WebGL2 float-texture
 *  upload path. Phase 2: that upload (`ensureSharedAtlasGpu` equivalent — creating + uploading the
 *  rgba32f curve/band textures) is not ported yet; in Phase 1 `SharedAtlas.gpu` stays `null` and
 *  only the CPU staging arrays (`curveTexData` / `bandTexData`) are built. */
