...
import { waitForTextureReady } from "../utils/textureUtils.js";
    ...
    public generateCommandsAndGatherInitPromises(initializationData: InitializationData, _finalOutput: boolean): void {
        ...
        for (const input of this._inputs) {
            if (input.type === ConnectionPointType.Texture) {
                const texture = input.runtimeData?.value as Nullable<GLTexture>;
                if (texture && !texture.isReady) {
                    // lite-gl exposes readiness as a `isReady` PROPERTY (not a method); poll until the
                    // async image upload completes (or a max-wait elapses) before declaring init done.
                    initializationData.initializationPromises.push(waitForTextureReady(initializationData.runtime.engine, texture));
                }
            }
        }
    }
