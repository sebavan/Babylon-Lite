import { type GLEffect, type GLEngineContext, type GLRenderTarget, setEffectTexture } from "babylon-lite-gl";
...
export class OutputBlock extends BaseBlock {
    ...
    /**
     * If supplied, the Smart Filter will render into this render target. Otherwise, it renders
     * into the canvas (default framebuffer) the lite-gl engine is using for rendering.
     *
     * Adaptation: this is a `babylon-lite-gl` {@link GLRenderTarget} (instead of Babylon's
     * `RenderTargetWrapper`).
     */
    public renderTargetWrapper: Nullable<GLRenderTarget> = null;
    ...
    public override generateCommandsAndGatherInitPromises(initializationData: InitializationData, finalOutput: boolean): void {
        ...
        const inputRuntimeData = this.input.runtimeData;
        if (this.input.connectedTo?.ownerBlock.isInput && inputRuntimeData) {
            const runtime = initializationData.runtime;

            const shaderBlockRuntime = new ShaderRuntime(runtime.engine, BlockShaderProgram, new OutputShaderBinding(inputRuntimeData));
...
class OutputShaderBinding extends ShaderBinding {
    ...
    public override bind(engine: GLEngineContext, effect: GLEffect): void {
        const texture = this._inputTexture.value;
        if (texture) {
            setEffectTexture(engine, effect, this.getRemappedName(Uniforms.input), texture);
        }
    }
}
