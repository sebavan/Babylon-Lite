...
/**
 * Class that controls the playing of lottie animations using `@babylonjs/lite-gl`.
 */
export class AnimationController {
     ...
     * @returns The canvas (an `HTMLCanvasElement`, or an `OffscreenCanvas` on the
     *  worker render path).
     */
    public get view(): HTMLCanvasElement | OffscreenCanvas {
        return getRenderingCanvas(this._engine);
    }
    ...
    public constructor(
        ...
        this._engine = createGLEngine(this._canvas, {
            alpha: true,
            stencil: false,
            antialias: false,
            depth: false,
        ...
        const maxTextureSize = this._engine.caps.maxTextureSize;
        this._rendererConfiguration = ResolveRendererConfiguration(configuration, maxTextureSize, mainThreadDevicePixelRatio);

        // Babylon's SpriteRenderer applies its own blend mode (ALPHA_COMBINE) every
        // frame, so no engine-wide alpha mode is set here; depth/stencil are disabled
        // at context creation (lite-gl has no depth/stencil attachment by default).
    ...
    public dispose(): void {
        ...
        const canvas = getRenderingCanvas(this._engine) as HTMLCanvasElement | OffscreenCanvas;
        if (canvas && (canvas as HTMLCanvasElement).remove) {
            (canvas as HTMLCanvasElement).remove();
        }

        this._renderingManager.dispose();
        for (const texture of this._spritePacker.textures) {
            disposeTexture(this._engine, texture);
        }
        disposeGLEngine(this._engine);
    }
    ...
    private _setSize(width: number, height: number, canvasScale: number): void {
        ...
        setGLEngineSize(_engine, width * canvasScale * devicePixelRatio, height * canvasScale * devicePixelRatio);
        ...
        _projectionMatrix.orthoOffCenterLeftHanded(
            0,
            getRenderWidth(_engine) / (devicePixelRatio * canvasScale),
            getRenderHeight(_engine) / (devicePixelRatio * canvasScale),
            0,
            -100,
            100
        );
    ...
    /**
     * Whether at least one frame has been drawn. The sprite shaders compile
     * asynchronously, so the very first {@link renderFrame} / playback frame may
     * clear without drawing sprites; this flips true once a frame has been issued.
     * @returns True once the first frame has rendered.
     */
    ...
    /**
     * Renders a single, specific animation frame deterministically, without
     * advancing the wall clock — the seek/scrub counterpart of the time-driven
     * playback loop. Every node is updated to `frame` and the layers are drawn
     * once. Used for static posters, thumbnails and deterministic parity
     * captures. Because the sprite shaders compile asynchronously, call this each
     * tick (e.g. in a short rAF loop) until the sprite atlas is ready if a
     * guaranteed-drawn frame is required.
     * @param frame - The animation frame to render (clamped to the animation's frame range).
     */
