import { type GLEngineContext, type GLTexture, createDynamicTexture, updateDynamicTexture, clearDynamicTextureSource } from "babylon-lite-gl";
...
export class SpritePacker {
     ...
     */
    public constructor(engine: GLEngineContext, isHtmlCanvas: boolean, atlasScale: number, variables: Map<string, string>, rendererConfiguration: LottieRendererConfig) {
        this._engine = engine;
    ...
    public updateAtlasTexture(): void {
        for (const page of this._pages) {
            ...
            updateDynamicTexture(this._engine, page.texture, page.canvas, false);
            page.isDirty = false;
        }
    }
    ...
    public releaseCanvas(): void {
        for (const page of this._pages) {
            // Drop the source the dynamic texture retained for context-restore replay
            // so the (large) atlas canvas can be garbage-collected. The GPU pixels are
            // kept; a post-release context loss re-blanks the atlas (as Babylon's does).
            clearDynamicTextureSource(page.texture);
    ...
    private _createPage(): AtlasPage {
        ...
        // Blank dynamic texture (linear filter, clamp wrap) matching Babylon's
        // createDynamicTexture(..., samplingMode 2 = BILINEAR) + ThinTexture wrapU/V = 0.
        const texture = createDynamicTexture(this._engine, this._rendererConfiguration.spriteAtlasWidth, this._rendererConfiguration.spriteAtlasHeight);
        updateDynamicTexture(this._engine, texture, canvas, false);
