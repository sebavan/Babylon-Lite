...
function ParseElements(context: LottieShapeLayerParseContext, elements: RawElement[] | undefined, parent: AnimationNode, rasterizationFrame: number): void {
    ...
    for (let i = 0; i < elements.length; i++) {
        ...
        if (el.ty === "gr") {
            ParseGroup(context, el, parent, rasterizationFrame, propagateDecorators ? levelDecorators : undefined);
            //break;
        } else if (el.ty === "sh" || el.ty === "rc" || el.ty === "el") {
            ParseShapes(context, elements, parent, rasterizationFrame);
            break; // After parsing the shapes, this array of elements is done
        } else if (propagateDecorators && (el.ty === "fl" || el.ty === "st" || el.ty === "gf" || el.ty === "gs")) {
            // Already absorbed into the preceding sibling groups via `ParseGroup` above.
            continue;
        } else {
            context.diagnostics.push(`Only groups or shapes are supported as children of layers - Name: ${el.nm} Type: ${el.ty}`);
            continue;
        }
    }
}
