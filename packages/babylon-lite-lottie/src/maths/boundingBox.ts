...
export function GetShapesBoundingBox(rawElements: RawElement[]): BoundingBox {
    ...
    for (let i = 0; i < rawElements.length; i++) {
        ...
        if (el.ty === "rc") {
            GetRectangleVertices(boxCorners, el as RawRectangleShape);
        } else if (el.ty === "el") {
            GetEllipseVertices(boxCorners, el as RawEllipseShape);
        } else if (el.ty === "sh") {
            GetPathVertices(boxCorners, el as RawPathShape);
        } else if (el.ty === "st") {
            strokeWidth = Math.max(strokeWidth, GetStrokeInset(el as RawStrokeShape));
        } else if (el.ty === "gs") {
            ...
            strokeWidth = Math.max(strokeWidth, GetStrokeInset(el as RawGradientStrokeShape));
        }
    }
